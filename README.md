...
- `lib/controller.ts`: one independent YouTube controller per enabled layer. The explicit launch click can call `playVideo()` on ready, visible players. No delayed retry is retained. Initial preparation, source replacement and layer re-enabling only cue; there is no page-load autoplay or automatic looping. Native Play remains the fallback.
